package net.tarpn.network;

import java.nio.ByteBuffer;
import net.tarpn.Util;
import net.tarpn.packet.impl.ax25.AX25Call;

public class NetRomConnect extends NetRom {

  private final byte proposedWindowSize;
  private final AX25Call originatingUser;
  private final AX25Call originatingNode;

  NetRomConnect(
      AX25Call originCall,
      AX25Call destCall,
      byte ttl,
      byte circuitIndex, byte circuitID,
      byte txSeqNumber, byte rxSeqNumber,
      byte opCode, byte windowSize, AX25Call originatingUser,
      AX25Call originatingNode) {
    super(originCall, destCall, ttl, circuitIndex, circuitID, txSeqNumber, rxSeqNumber, opCode);
    this.proposedWindowSize = windowSize;
    this.originatingUser = originatingUser;
    this.originatingNode = originatingNode;
  }

  public byte getProposedWindowSize() {
    return proposedWindowSize;
  }

  public AX25Call getOriginatingUser() {
    return originatingUser;
  }

  public AX25Call getOriginatingNode() {
    return originatingNode;
  }

  @Override
  public void write(ByteBuffer buffer) {
    super.write(buffer);
    buffer.put(proposedWindowSize);
    originatingUser.write(buffer::put);
    originatingNode.write(buffer::put);
  }

  @Override
  public String toString() {
    return "NetRomConnect{" +
        "originCall=" + getOriginCall() +
        ", destCall=" + getDestCall() +
        ", ttl=" + Util.toHexString(getTtl()) +
        ", circuitIndex=" + Util.toHexString(getCircuitIndex()) +
        ", circuitID=" + Util.toHexString(getCircuitID()) +
        ", txSeqNumber=" + Util.toHexString(getTxSeqNumber()) +
        ", rxSeqNumber=" + Util.toHexString(getRxSeqNumber()) +
        ", opCode=" + getOpType() +
        ", proposedWindowSize=" + getProposedWindowSize() +
        ", originatingUser=" + getOriginatingUser() +
        ", originatingNode=" + getOriginatingNode() +
        '}';
  }
}
